...
/*
 * instrument_mem is called whenever a memory reference is identified. It
 * inserts code before the memory reference to to fill the memory buffer and
 * jump to our own code cache to call the clean_call when the buffer is full.
 */
static void
               ...
               int pos, bool write)
{
  ...
  /* 
   * Steal the register for memory reference address * We can optimize away
   * the unnecessary register save and restore by analyzing the code and
   * finding the register is dead.
   */
  ...
  /*
   * we use lea + jecxz trick for better performance lea and jecxz won't disturb
   * the eflags, so we won't insert code to save and restore application's
   * eflags.
   */
